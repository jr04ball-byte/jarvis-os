...
- The primary `/dashboard` is the V25 neon command center interface: fixed left navigation, greeting/weather/time header, active-task and recent-activity panels, provider cards, calendar, lower email/projects/agents/system panels, and a central animated neural sphere. The animated centerpiece is a Blender-rendered scene with orbiting traces, drifting signal lights and pulsing beams. A motion toggle and reduced-motion fallback are included. The V23 overhead dashboard, compatibility route, bake scripts, and `command-center`, `orb`, and `bg` media were removed.
...
- Push-to-talk transcription can run fully locally with optional Faster Whisper. Install `api-gateway/requirements-voice-local.txt`, set `VOICE_TRANSCRIBE_PROVIDER=local`, and choose the model/device through the documented `WHISPER_*` environment settings. Gemini fallback occurs only when `VOICE_TRANSCRIBE_GEMINI_FALLBACK=true`. This does not enable continuous listening or spoken output.
- An isolated local voice lab is available at `/voice-duplex` when `VOICE_DUPLEX_ENABLED=true`. It uses continuous browser PCM capture, local Faster Whisper, the configured Ollama or Gemini brain, and optional Kokoro v1.0 speech. It is disabled by default, starts only after a user gesture, stops on page hiding, preserves text responses when audio fails, and leaves the production push-to-talk path unchanged. Model weights remain under runtime `data` and are excluded from Git and release archives.
- Live Gemini token and Deepgram speech-output endpoints return HTTP 410. Existing legacy voice source files remain for historical compatibility tests but are not served by the voice routes. No camera, Kinect, gestures, voice-to-voice mode, or overhead dashboard is included.
